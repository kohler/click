...
static int
cp_va_parsev(Vector<String> &args, ErrorHandler *errh, va_list val)
{
  int argno = 0;
  ...
  if (too_few_args || argno < args.size()) {
    ...
    errh->error("too %s arguments to configuration, expected `%s'", whoops,
		signature.cc());
  }
...
int
cp_va_parse(const String &argument, ErrorHandler *errh, ...)
{
  va_list val;
  va_start(val, errh);
  Vector<String> args;
...
int
cp_va_parse(Vector<String> &args, ErrorHandler *errh, ...)
{
  va_list val;
  va_start(val, errh);
  int retval = cp_va_parsev(args, errh, val);

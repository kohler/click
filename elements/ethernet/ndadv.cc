/*
 * ndadv.{cc,hh} -- element that responds to Neighborhood Solitation Msg
 * Peilei Fan
 *
...
void
NDAdv::add_map(IP6Address ipa, IP6Address mask, EtherAddress ena)
{
  struct Entry e;
...
int
NDAdv::configure(const Vector<String> &conf, ErrorHandler *errh)
{
  _v.clear();
...
Packet *
NDAdv::make_response(u_char dha[6],   /*  des eth address */
...
Packet *
NDAdv::make_response2(u_char dha[6],   /*  des eth address */
			     u_char sha[6],   /*  src eth address */
			     u_char dpa[16],  /*  dst IP6 address */
			     u_char spa[16],  /*  src IP6 address */
			     u_char tpa[16])  /*  target IP6 address */
...
bool
NDAdv::lookup(IP6Address a, EtherAddress &ena)
{
  int i, besti = -1;
...
EXPORT_ELEMENT(NDAdv)

// generate Vector template instance
#include "vector.cc"
template class Vector<NDAdv::Entry>;

...
print "// This configuration was generated by make-flexcliet-ron.pl\n";
print "// $mon/$mday/$year  $hour:$min:$sec\n";
print "//\n";
print "// make-flexcliet-ron.pl ",$device, " ",$meIP, " ", $meHW, " ", $gwIP, " ";
for($i=0; $i<$n; $i++) {
    print $servers[$i], " ";
}
print "\n//\n";
...
print "\n// Classify outgoing packets by source port\n";
print "portclass :: PortClassifier(60000, 100);\n";
...
for($i=0; $i<$n; $i++) {
    ...
    print "\t-> CheckIPHeader\n";
    print "\t-> GetIPAddress(16)\n";
    print "\t-> p$i :: PortClassifier(60000, ", 100 * $numpolicy, ")\n";
    print "\t-> [", $i+2, "]rt;\n";
    print "\n";
}
...
print "iprw[0] -> portclass;\n";
for($i=0; $i<$n+$numpolicy+1; $i++) {
    if ($i < $numpolicy) {
	print "portclass[$i] -> Paint($i) -> [0]rt;\n";
    } elsif ($i == $numpolicy) {
	print "portclass[$i] -> setgw;\n";
    } else {
	print "portclass[$i]\n\t->IPFragmenter(1400, false)\n";
	print "\t-> UDPIPEncap($meIP, 4000, ",$servers[$i-$numpolicy-1], ", 4000)\n";
	print "\t-> setgw;\n";
    }
}
print "\n";

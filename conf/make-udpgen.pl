...
my $rtrifs = [ 
	[ "00:C0:95:E1:FC:D7" ], # 2.0.0.1
	[ "00:C0:95:E1:FC:D6" ], # 3.0.0.1
	[ "00:C0:95:E1:FC:D5" ], # 4.0.0.1
	[ "00:C0:95:E1:FC:D4" ], # 5.0.0.1
	[ "00:C0:95:E2:09:15" ], # 6.0.0.1
	[ "00:C0:95:E2:09:14" ], # 7.0.0.1
	[ "00:C0:95:E2:16:9D" ], # 8.0.0.1
	[ "00:C0:95:E2:16:9C" ], # 9.0.0.1
];
...
my $cltifs = [
	...
	[ "00:00:C0:8A:67:EF", "eth2" ], # 8.0.0.2
	[ "00:A0:CC:55:E3:D0", "eth1" ], # 9.0.0.2
];

if ($#ARGV != 3) {
  print "usage: make-updgen.pl src dest rate time\n";
  print "   where src and dest are network numbers between 2 and 9\n";
  exit();
}
...
ar	:: ARPResponder($s.0.0.2 255.255.255.255 $cltifs->[$s-2]->[0]);
iph	:: IPEncap(17, $s.0.0.2, $d.0.0.2);
ethh	:: EtherEncap(0x800, $cltifs->[$s-2]->[0], $rtrifs->[$s-2]->[0]);
c0	:: Classifier(12/0806 20/0001, -);
pd	:: PollDevice($cltifs->[$s-2]->[1]);
td	:: ToDevice($cltifs->[$s-2]->[1]);
...
rs :: RatedSource(\\<00000000111111112222222233333333444444445555>, $r, $t);
